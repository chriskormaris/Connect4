package com.chriskormaris.connect4.gui.util;


import lombok.AccessLevel;
import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor(access = AccessLevel.PRIVATE)
public class GuiConstants {

	public static final String CONNECT_4_TITLE = "Connect-4";
	public static final String CONNECT_5_TITLE = "Connect-5";

	public static final String VERSION = "2.8.1";

	public static final String CONNECT_4_BOARD_IMG_PATH = "images/Board4.png";
	public static final String CONNECT_5_BOARD_IMG_PATH = "images/Board5.png";

	public static final int DEFAULT_CONNECT_4_WIDTH = 600;
	public static final int DEFAULT_CONNECT_4_HEIGHT = 625;

	public static final int DEFAULT_CONNECT_5_WIDTH = 670;
	public static final int DEFAULT_CONNECT_5_HEIGHT = 700;

}
